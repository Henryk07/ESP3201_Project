...
def augmentationdata(n, directorypath, sourcedirectory):  # enter source directory
    ...
    for pathdir in glob.glob(sourcedirectory+"/*"):
        ...
        if (len(img_path_list)) > n:
            ...
            while i < int(abs(n/2)):
                ...
                if img_path_list[ran] not in randused:
                    ...
                    img.save(directorypath+"/S_datas_labels_augmented" +
                             str(n)+"/"+dirlab+"//"+rootpath)
                    img_path_list_dest = glob.glob(
                        directorypath+"/S_datas_labels_augmented"+str(n)+"/"+dirlab+"/*.png")
        else:
            # copy all existing images to new directory
        ...
        while len(img_path_list_dest) != n:  # complete new directory with augmented images
            ...
            # Saving the image
            augarray.save(directorypath+"/S_datas_labels_augmented" +
                          str(n)+"/"+dirlab+"//"+rootpath+"aug"+str(j)+".png")
            img_path_list_dest = glob.glob(
                directorypath+"/S_datas_labels_augmented"+str(n)+"/"+dirlab+"/*.png")
...
augmentationdata(5000, "M_dataset", "Full_dataset/ECG_Image_data_cb_bw")
